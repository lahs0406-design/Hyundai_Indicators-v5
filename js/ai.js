...
var AI_SYSTEM_PROMPT = [
  ...
  '',
  '[분석 내용 및 답변 구성 — 반드시 이 순서와 형식을 지켜주세요]',
  '',
  '1. **총론** — 문장형 4~5줄. 각 지표의 최근 3~4개월 방향성(상승/하락/보합)과 변화 폭을 수치와 함께 요약',
  '2. **지표별 흐름** — 지표마다 "· 지표명: 최신값, 전월비, 전년비 등 핵심 수치" 형태의 글머리표(·) 목록, 지표당 1~2문장',
  '3. **지표간 상관관계** — 문장형 2~3줄 (아래 유의사항 예시 참조). 환율처럼 긍정·부정 효과가 동시에 있는 지표는 한쪽만 쓰지 말고 양면을 모두 짚을 것',
  '4. **당사 상품군에 미치는 영향** — 표 (컬럼: 구분 | 기회요인 | 리스크요인, 수치 근거는 각 문장 안에 포함). 상품군별 한 행씩, 기회·리스크요인은 각 1~2문장으로 서술하고 문장 앞에 대상 고객군("(내국인 VIP고객)" / "(내국인 일반고객)" / "(외국인 관광객)")을 괄호로 명시. 너무 구체적인 방안은 삼가 (프로모션 전개 같은)',
  '5. **결론** — 문장형 1~2줄',
  '',
  '※ 위 1~5번 항목마다 소제목을 굵게 달고, 항목과 항목 사이에는 구분선(---)을 넣어 구분할 것',
  "※ 문장형 항목(총론, 상관관계, 결론)은 모두 '존대어(~입니다 등)' 형태로 작성",
  '※ 숫자 표기시 단위까지 표기해주고, 소수점이 있는경우 첫째자리까지 표기',
  '※ 지표별 흐름 목록과 상품군 영향 표를 제외한 나머지는 평서문 형태 (한국어) 로만 작성',
  '',
  '[인사이트 작성시 유의사항]',
  '',
  ...
  '    Ex) 금리 상승 / 물가상승 동반 → 내국인 고객의 소비여력 감소 → 비필수재·사치재 수요 감소 대응 필요, 필수재 및 가격 소구 중심의 상품 전개 필요',
  '',
  '※ 단순 현황 나열이 아닌, 수치 기반 판단 근거를 포함할 것',
  '※ 긍정/부정 양면을 균형 있게 서술할 것',
  '※ 특히 환율 하락(원화 강세)은 ① 수입 브랜드·상품의 원가 절감이라는 긍정적 효과와, ② 외국인 관광객 입장에서 원화 표시 가격이 상대적으로 비싸져 가격 소구력(구매 메리트)이 감소하는 부정적 효과가 동시에 존재함. 두 측면을 함께 언급할 것',
...
/* ===========================================
   ai.js § 0-1. 교차분석 답변 예시 (few-shot)
   · buildDefaultPrompt()에서 유저 메시지 끝에 그대로 첨부되어,
     LLM이 [답변 구성] 지시문만으로는 놓치기 쉬운 톤·분량·표 형태를
     실제 예시로 보고 따라가도록 함.
   · 예시 안의 구체적 수치·문장은 참고용일 뿐이며, 실제 응답에는
     그대로 베끼지 말고 제공된 실제 데이터로 새로 작성하도록 별도 지시함.
   =========================================== */
var CROSS_ANALYSIS_EXAMPLE = [
  '**총론**',
  '',
  '최근 4개월 기준 소비심리지수는 106.1(5월) → 106.6(6월) → 106.8(7월) → 104.5(8월)로 기준선 100은 상회하고 있으나 8월 들어 2.3p 하락하며 소비심리가 다소 둔화되었습니다. 기준금리는 2.5%에서 7월 2.75%, 8월 3.0%로 0.5%p 상승하며 긴축 기조가 강화되었습니다.',
  '',
  '소비자물가 상승률은 최근 2.8~3.2% 수준으로 높은 물가 부담이 지속되고 있으며, 환율은 6월 1,541.5원에서 8월 1,376.5원으로 165.0원 하락해 원화 강세가 나타났습니다.',
  '',
  '반면 코스피는 6월 말 약 3,071.7pt에서 8월 말 3,186.0pt 수준으로 상승세를 유지했고, 외국인 관광객은 6월 199.3만 명 → 7월 209.3만 명으로 증가하며 방한 수요가 확대되었습니다.',
  '',
  '---',
  '',
  '**지표별 흐름**',
  '',
  '· 소비심리지수(CSI): 5~7월 106~107 수준의 안정적 흐름 이후 8월 104.5로 하락하며 소비심리 개선세가 둔화되었습니다.',
  '· 소비자물가(CPI): 5~8월 전년동월 대비 약 +3.1% → +3.2% → +2.8% → +3.1% 수준으로 물가 부담이 지속되고 있습니다.',
  '· 기준금리: 5~6월 2.5% 동결 이후 7월 2.75%, 8월 3.0%로 연속 인상되었습니다.',
  '· 환율(원/달러): 6월 1,541.5원 → 7월 1,441.1원 → 8월 1,376.5원으로 큰 폭 하락하며 원화 강세가 나타났습니다.',
  '',
  '---',
  '',
  '**지표간 상관관계**',
  '',
  '코스피 상승과 소비심리지수의 100 이상 유지는 내국인 VIP 고객 중심의 자산효과를 통해 명품·하이주얼리·프리미엄 패션 수요에 긍정적으로 작용하는 환경입니다. 다만 8월 CSI 하락은 소비 확대 속도 둔화를 시사합니다.',
  '',
  '환율 하락(1,541.5원 → 1,376.5원)과 외국인 관광객 증가는 방한 수요 자체는 확대되고 있으나 외국인의 원화 체감 구매력은 일부 약화되는 구조입니다. 외국인 매출은 증가 가능성이 있으나 가격 메리트는 과거 대비 낮아질 수 있습니다.',
  '',
  '금리 인상과 물가 3% 내외 지속은 내국인 일반고객의 실질 구매력을 제약하여 비필수재 소비 확대에 부담 요인으로 작용할 가능성이 있습니다.',
  '',
  '---',
  '',
  '**당사 상품군에 미치는 영향**',
  ...
  '| 구분 | 기회요인 | 리스크요인 |',
  '|---|---|---|',
  '| 패션·SPA | (내국인 VIP·일반고객) 소비심리지수 100 이상 유지 및 코스피 상승에 따른 구매 수요 유지 가능성 | (내국인 일반고객) 8월 CSI 하락(-2.3p), 금리 3.0%로 상승하며 중가 의류 구매 둔화 가능성 |',
  '| 명품·하이주얼리 | (내국인 VIP고객) 코스피 약 114pt 상승에 따른 자산효과 확대, (외국인 관광객) 200만 명 이상 유입 지속 | (외국인 관광객) 원화 강세로 체감 가격 경쟁력 일부 약화 |',
  '| 리빙·가전 | (내국인 VIP고객) 코스피 상승에 따른 프리미엄 리빙 및 고관여 상품 관심 확대 가능 | (내국인 일반고객) 금리 3.0%, 물가 3% 내외 지속으로 대형 구매 의사결정 지연 가능성 |',
  '| F&B·식품관 | (외국인 관광객) 관광객 증가에 따른 식음 수요 확대, 경기 변동에도 상대적 방어력 보유 | (내국인 일반고객) 물가 부담 지속으로 객단가 상승에 대한 저항 가능성 |',
  '',
  '---',
  '',
  '**결론**',
  '',
  '현재는 자산효과(코스피 상승)와 관광객 증가가 백화점 수요를 지지하고 있으나, 금리 인상·물가 부담·소비심리 둔화가 동반되고 있어 VIP 중심 럭셔리 수요는 견조하되 일반 고객의 비필수재 소비는 점진적으로 약화될 가능성에 유의할 필요가 있습니다.'
].join('\n');
...
function buildDefaultPrompt(checked) {
  ...
  return '아래 경제 지표 데이터를 바탕으로 현대백화점 상품본부 관점에서 인사이트를 작성해 주세요.\n\n' +
    '분석 지표: ' + labels.join(', ') + '\n\n' +
    '다음 구성 순서와 형식으로 작성해 주세요 (시스템 프롬프트의 [답변 구성]을 그대로 따르세요):\n' +
    '1. 총론 — 문장형 4~5줄. 각 지표의 최근 3~4개월 방향성(상승/하락/보합)과 변화 폭을 수치와 함께 요약해 주세요.\n' +
    '2. 지표별 흐름 — 지표마다 "· 지표명: 핵심 수치" 형태의 글머리표 목록으로, 지표당 1~2문장씩 작성해 주세요.\n' +
    '3. 지표간 상관관계 — 문장형 2~3줄.\n' +
    '4. 당사 상품군에 미치는 영향 — 반드시 표로 작성해 주세요 (컬럼: 구분 | 기회요인 | 리스크요인, 수치 근거는 각 문장 안에 포함).\n' +
    '   - 기회요인·리스크요인은 각각 1~2문장으로 조금 더 구체적으로 서술하고, 문장 앞에 어떤 고객군(내국인 VIP고객 / 내국인 일반고객 / 외국인 관광객)에게 해당하는 영향인지 "(외국인 관광객)"처럼 괄호로 명시해 주세요.\n' +
    '5. 결론 — 문장형 1~2줄.\n\n' +
    '위 1~5번 항목마다 "총론", "지표별 흐름", "지표간 상관관계", "당사 상품군에 미치는 영향", "결론"을 굵은 소제목으로 달아주시고, 항목과 항목 사이에는 구분선(---)을 넣어 시각적으로 구분해 주세요.\n' +
    '문장형 항목(총론, 상관관계, 결론)은 모두 존대어(~입니다 등)로 작성해 주세요.\n' +
    '환율은 양면적인 지표입니다 — 환율이 하락(원화 강세)했다면 ① 수입 브랜드·상품의 원가 절감 효과와 ② 외국인 관광객 입장에서 원화 표시 가격이 상대적으로 비싸져 가격 소구력(구매 메리트)이 감소하는 효과를 모두 짚어 주세요. 한쪽 효과만 쓰지 말아 주세요.\n' +
    '숫자 표기 시 단위까지 표기하고, 소수점이 있는 경우 첫째자리까지 표기해 주세요.\n' +
    '날짜나 시점을 언급할 때는 반드시 "OO년 O월"(예: 26년 7월) 형식으로만 표기하고, "26.07" 같은 표기는 쓰지 마세요.\n\n' +
    '[답변 예시 — 구성·분량·톤·서식을 아래와 유사하게 맞춰 작성해 주세요. 예시 속 수치·문장은 참고용일 뿐이니 그대로 베끼지 말고, 실제로 제공되는 [데이터] 섹션의 값으로 새로 작성해 주세요]\n' +
